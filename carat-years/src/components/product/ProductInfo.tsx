  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="flex flex-col gap-10 lg:flex-row">
        {/* LEFT : MEDIA */}
        <div className="relative flex flex-col items-center lg:w-1/2 mt-12">
          ...
          <div className={`mt-6 flex w-full gap-4 overflow-x-auto no-scrollbar ${isFewImages ? "justify-center" : ""}`}>
            {allImages.map((src, idx) => (
              <div key={idx} onClick={() => setCurrentIndex(idx)} className={`h-40 w-40 shrink-0 cursor-pointer rounded-lg border ${currentIndex === idx ? "border-2 border-[#957127]" : "border-[#957127]"}`}>
        ...
        <div className="flex flex-col gap-6 lg:w-1/2">
          ...
          <div className="rounded-2xl border border-[#E8D9B5] bg-linear-to-br from-[#F8F2E7] to-[#F4E8D6] p-5 shadow-md sm:p-6">
            <div className="flex justify-end gap-2">
              <Button variant="outline" className="rounded-full p-2.5"><Share2 className="h-4 w-4" /></Button>
              <Button variant="outline" className="rounded-full p-2.5"><Heart className="h-4 w-4" /></Button>
            </div>
            <p className="mt-3 text-xs font-medium uppercase tracking-wide text-gray-600">Standard Price</p>
            
            {discountPreview?.appliedCode ? (
              <div className="mt-3 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-gray-500 line-through">₹{discountPreview.originalPrice.toLocaleString()}</p>
                  <div className="mt-1 flex flex-wrap items-end gap-2">
                    <p className="text-3xl font-bold text-[#351043] sm:text-4xl">₹{discountPreview.finalPrice.toLocaleString()}</p>
                    <span className="rounded-full bg-green-600 px-3 py-1 text-xs font-semibold text-white">SAVE ₹{discountPreview.totalDiscount.toLocaleString()}</span>
                  </div>
                  <p className="mt-1 text-sm text-green-700">You saved ₹{discountPreview.totalDiscount.toLocaleString()}</p>
                </div>
                <div className="flex items-center gap-2 sm:justify-end sm:min-w-[200px]">
                  <span className="inline-flex items-center rounded-md bg-green-100 px-3 py-1 text-xs font-semibold text-green-800">{discountPreview.appliedCode}</span>
                  <button onClick={() => { navigator.clipboard.writeText(discountPreview.appliedCode); toast.success("Coupon code copied"); }} className="rounded-md border border-green-300 bg-white px-3 py-1 text-xs font-semibold text-green-700 hover:bg-green-50">Copy</button>
                </div>
              </div>
            ) : (
              /* FIXED HERE: Use selectedSizeObj.grossValue instead of selectedCaratObj.totalAmt */
              <p className="mt-3 text-3xl font-bold text-[#351043] sm:text-4xl">
                ₹{(selectedSizeObj?.grossValue ?? 0).toLocaleString()}
              </p>
            )}
          </div>
          ...
          <div className="hidden lg:grid gap-3"> <Button className="border-2 border-[#957127] hover:bg-white hover:text-black">Any query - Book Appointment</Button> </div>
        </div>
      </div>
    </div>
  );
